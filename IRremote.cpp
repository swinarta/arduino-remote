...
void IRsend::sendRCMM(unsigned long data, int nbits)
{
  ...
  for (int i = 0; i < nbits/2; i++)
  {
      if(data & TOPBIT)
      {
        if(data & LOWERBIT)
        {
        	//11
        	mark(RCMM_BIT_MARK);
        	space(RCMM_THREE_SPACE);
        }else{
        	//10
        	mark(RCMM_BIT_MARK);
        	space(RCMM_TWO_SPACE);
        }
      }else{
        if(data & LOWERBIT)
        {
        	//01
            mark(RCMM_BIT_MARK);
            space(RCMM_ONE_SPACE);
        }else{
        	//00
            mark(RCMM_BIT_MARK);
            space(RCMM_ZERO_SPACE);
        }
      }
  ...
  mark(RCMM_BIT_MARK);
  space(RCMM_RPT_LENGTH);

  space(0); // Turn off at end
}

void IRsend::sendMitsubishiAC(unsigned int data1, unsigned long data2, unsigned int data3, unsigned int data4, unsigned int data5){
	enableIROut(38);
